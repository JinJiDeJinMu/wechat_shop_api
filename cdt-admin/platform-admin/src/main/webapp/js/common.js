 ...
 * 维护状态 1：未维护 2:已维护
 */
function transUpkeepState(upkeepState) {
    if (upkeepState == 1) {
        return '未维护';
    }
    if (upkeepState == 2) {
 ...
 * 客户属性 1:客户 2：准客户
 */
function transCustomerState(customerState) {
    if (customerState == 1) {
        return '客户';
    }
    if (customerState == 2) {
 ...
 */
function transDataFrom(dataFrom) {
    if (dataFrom == 1) {
        return '用户购买商品,邮寄给客户';
    }
    if (dataFrom == 2) {
        return '用户买了服务性商品后，系统插入';
    }
    if (dataFrom == 3) {
        return '用户自行维护';
    }
